...
class LongReadsTestSuite
  ...
  test("BAM - coverage - Nanopore with guppy basecaller") {
    ...
    val query =
      s"SELECT contigName, start, coverage FROM bdg_coverage('$tableNameBAM','nanopore_guppy_slice','bases') order by contigName,start,end"
    ...
    assert(
      covMultiPartitionDF
        .where("contigName='chr21' and start == 5010515")
        .first()
        .getShort(2) == 1) // value check [first element]
    assert(
      covMultiPartitionDF
        .where("contigName='chr21' and start == 5022667")
        .first()
        .getShort(2) == 15) // value check [partition boundary]
    assert(
      covMultiPartitionDF
        .where("contigName='chr21' and start == 5036398")
        .first()
        .getShort(2) == 14) // value check [partition boundary]
    assert(
      covMultiPartitionDF
        .where("contigName='chr21' and start == 5056356")
        .first()
