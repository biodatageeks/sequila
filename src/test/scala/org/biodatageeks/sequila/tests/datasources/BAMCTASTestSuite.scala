...
//      .explain(true)
//
//    ss.sql(s"DESC FORMATTED  bam_ctas")
//      .show(1000, false)
//
//    val dfSrc = ss.sql(
//      s"SELECT contigName,start,end FROM ${tableNameBAM} WHERE contigName='chr1' AND start>390 ORDER BY contigName, start")
//    dfSrc.show(25)
//    //println(dfSrc.count())
//    val dfDst = ss.sql(
//      s"SELECT contigName,start,end FROM bam_ctas WHERE contigName='chr1' AND start>390 ORDER BY contigName, start")
//    //println(dfDst.count())
//    dfDst.show(25)
//    println(s"src ${dfSrc.count} dst ${dfDst.count}")
...
//    SequilaRegister.register(ss)
//    ss.sql(s"""
//           |CREATE TABLE IF NOT EXISTS bam_ias USING org.biodatageeks.sequila.datasources.BAM.BAMDataSource
//           |OPTIONS(path "${bamIAS}/*.bam")
//        """.stripMargin)
//      //  .show()
//      .explain(true)
//    ss.sql(s"INSERT INTO bam_ias SELECT * FROM ${tableNameBAM}")
//      .explain(true)
//    //  .show
//
//    val dfSrc = ss.sql(
//      s"SELECT contigName,start,end FROM ${tableNameBAM} ORDER BY contigName, start")
//    println(dfSrc.count())
//    val dfDst = ss.sql(
//      s"SELECT contigName,start,end FROM bam_ias ORDER BY contigName, start")
//    println(dfDst.count())
//    assertDataFrameEquals(dfSrc, dfDst)
//
//  }
...
//      .explain(true)
//    ss.sql(
//        s"INSERT OVERWRITE TABLE bam_ias SELECT * FROM ${tableNameBAM} limit 10")
//      .explain(true)
//    //.show
//    val dfDst = ss.sql(
//      s"SELECT contigName,start,end FROM bam_ias ORDER BY contigName, start")
