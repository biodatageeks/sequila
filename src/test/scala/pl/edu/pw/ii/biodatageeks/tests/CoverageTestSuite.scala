...
class CoverageTestSuite extends FunSuite with DataFrameSuiteBase with BeforeAndAfter with SharedSparkContext{
    ...
    val cramMultiPath = getClass.getResource("/multichrom/cram/NA12878.multichrom.cram").getPath
    val refMultiPath = getClass.getResource("/multichrom/NA12878.multichrom.fasta").getPath
    val adamPath = getClass.getResource("/NA12878.slice.adam").getPath
    val metricsListener = new MetricsListener(new RecordedMetrics())
    val writer = new PrintWriter(new OutputStreamWriter(System.out))
    val tableNameBAM = "reads"
    val tableNameMultiBAM = "readsBAMMulti"
    val tableNameMultiCRAM = "readsCRAMMulti"
    ...
    before{
      ...
      spark.sql(s"DROP TABLE IF EXISTS ${tableNameMultiCRAM}")
      spark.sql(
        s"""
           |CREATE TABLE ${tableNameMultiCRAM}
           |USING org.biodatageeks.datasources.BAM.CRAMDataSource
           |OPTIONS(path "${cramMultiPath}", refPath "${refMultiPath}")
           |
      """.stripMargin)
  ...
  test("CRAM - bdg_coverage - windows"){
    ...
    session.sql(s"desc formatted ${tableNameMultiCRAM}").show(100,false)
    val windowLength = 100
    val bdg = session.sql(s"SELECT * FROM bdg_coverage('${tableNameMultiCRAM}','NA12878', 'blocks', '${windowLength}')")
    ...
    test("CRAM - bdg_coverage - blocks - allPositions"){
      spark.sqlContext.setConf(BDGInternalParams.InputSplitSize, splitSize)
      ...
      val bdg = session.sql(s"SELECT * FROM bdg_coverage('${tableNameMultiCRAM}','NA12878', 'blocks')")
    ...
    test("CRAM - bdg_coverage - bases - notAllPositions"){
      spark.sqlContext.setConf(BDGInternalParams.InputSplitSize, splitSize)
      val session: SparkSession = SequilaSession(spark)
      SequilaRegister.register(session)

      session.sqlContext.setConf(BDGInternalParams.ShowAllPositions,"false")
      val bdg = session.sql(s"SELECT contigName, start, end, coverage FROM bdg_coverage('${tableNameMultiCRAM}','NA12878', 'bases')")
