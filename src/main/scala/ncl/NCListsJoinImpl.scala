...
object NCListsJoinImpl extends Serializable {
  ...
  def overlapJoin(sc: SparkContext,
                  rdd1: RDD[(Interval[Long], InternalRow)],
                  rdd2: RDD[(Interval[Long], InternalRow)]): RDD[(InternalRow, Iterable[InternalRow])] = {
    ...
    val nclist = sc.broadcast(new NCListTree[Long](localIntervals.toList))
    val kvrdd2: RDD[(Long, Iterable[InternalRow])] = rdd2
      // join entry with the intervals returned from the interval tree
      .map(x => (nclist.value.getAllOverlappings(x._1), x._2))
      ...
      .groupByKey
    indexedRdd1 // this is RDD[(Long, (Interval[Long], Row))]
