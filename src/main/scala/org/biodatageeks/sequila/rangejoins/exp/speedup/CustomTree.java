...
public class CustomTree<V> implements BaseIntervalHolder<V> {
	...
	public class CustomIterator implements Iterator<BaseNode<V>> {
		...
		private int nextOverlap() {
			if (qNext < firstOverlap) {
				int nextNodeIndex = qNext;
				while (stillPossibleOnLeftOverlap(nextNodeIndex)) {
					nextNodeIndex--;
					if (intervals.get(nextNodeIndex).overlaps(qStart,qEnd)) {
						return nextNodeIndex;
					}
				}
				return -1;
			}
			int nextNodeIndex = qNext + 1;
			if (isImaginary(nextNodeIndex)) {
				return -1;
			} else {
				if (intervals.get(nextNodeIndex).overlaps(qStart,qEnd)) {
					return nextNodeIndex;
				...
				while(stillPossibleOnRightOverlap(nextNodeIndex)) {
					if (intervals.get(nextNodeIndex).overlaps(qStart,qEnd)) {
						return nextNodeIndex;
					}
					nextNodeIndex++;
				}
				// right scan is finished
				nextNodeIndex = firstOverlap -1;

				if(nextNodeIndex>=0 && intervals.get(nextNodeIndex).overlaps(qStart, qEnd)) {
					return nextNodeIndex;
				}
		...
		private boolean stillPossibleOnRightOverlap(int nodeIndex) {
			return !isImaginary(nodeIndex) && intervals.get(nodeIndex).getStart() <= qEnd;
		}
