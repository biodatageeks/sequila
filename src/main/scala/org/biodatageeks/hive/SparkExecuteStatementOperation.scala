...
import org.apache.spark.sql.{DataFrame, SQLContext, SequilaSession, Row => SparkRow}
...
//  SequilaRegister.register(ss)
  ...
  def close(): Unit = {
    ...
    ss.sqlContext.sparkContext.clearJobGroup()
  }
  ...
  def getNextRowSet(order: FetchOrientation, maxRowsL: Long): RowSet = {
    ...
    if (order.equals(FetchOrientation.FETCH_FIRST)) {
      iter = if (ss.sqlContext.getConf(SQLConf.THRIFTSERVER_INCREMENTAL_COLLECT.key).toBoolean) {
        resultList = None
  ...
  private def execute(): Unit = {
    ...
    ss.sqlContext.sparkContext.setJobGroup(statementId, statement)
    val pool = sessionToActivePool.get(parentSession.getSessionHandle)
    if (pool != null) {
      ss.sqlContext.sparkContext.setLocalProperty("spark.scheduler.pool", pool)
    }
    try {

      result= ss.sql(statement)
  ...
  private def cleanup(state: OperationState) {
    ...
    if (statementId != null) {
      ss.sqlContext.sparkContext.cancelJobGroup(statementId)
    }
  }
}
